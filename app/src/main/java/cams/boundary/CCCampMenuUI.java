...
public class CCCampMenuUI extends BaseUI{
    ...
    private Boolean registerForCamp() {
        ...
        if (IDHelper.getCampFromID(campChoice).getLeavers().contains(MainApp.currentUser.getID())) {
        ...
        switch (registerChoice) {
            case 1:
                IDHelper.getCampFromID(campChoice).addAttendee(MainApp.currentUser.getID());
            ...
            case 2:
                ...
                IDHelper.getCampFromID(campChoice).addCommittee(MainApp.currentUser.getID());
    ...
    private void withdrawFromCamp() {
        ...
        int index = 1;
        for (Camp camp : MainApp.camps) {
            if (camp.getListOfAttendees().contains(MainApp.currentUser.getID()) || camp.getListOfCampCommittees().contains(MainApp.currentUser.getID())) {
        ...
        if (selectedCampNumber >= 1 && selectedCampNumber <= index - 1) {
            ...
            if (selectedCamp != null && (selectedCamp.getListOfAttendees().contains(MainApp.currentUser.getID()) || selectedCamp.getListOfCampCommittees().contains(MainApp.currentUser.getID()))) {
                // Withdraw the student from the camp
                selectedCamp.getListOfAttendees().remove(MainApp.currentUser.getID());
    ...
    private void viewRegisteredCamps() {
        ...
        for (Camp camp : MainApp.camps) {
            ...
            if (camp.getListOfAttendees().contains(currentStudent.getID()) || camp.getListOfCampCommittees().contains(currentStudent.getID())) {
